...
public:
    ...
    long long getRes(list<string>& ls){
        long long res  = 0;
        int flag = 1;
        string op;
        while(!ls.empty()){
            ...
            if(op == "+"){
                ...
                res  += flag?stol(op):-stol(op);
            }
        }
        return res;
    }
    void pushNum(long long num, list<string>& ls){
        if(!ls.empty()){
            string op = ls.back();
            if(op == "*" || op =="/"){
                ls.pop_back();
                num = op=="*"?stol(ls.back())*num:stol(ls.back())/num;
                ls.pop_back();
            }
        }
        ls.push_back(to_string(num));
    }
    void debug(list<string>& ls){
        while(!ls.empty()){
            cout<<ls.front()<<" ";
            ls.pop_front();
        }
